    ...
    const fetchUserData = async () => {
      try {
        ...
        bookingSnapshot.forEach(doc => {
          ...
          // Check if booking has ended but status is still active
          if (booking.status === 'active' && new Date(booking.endTime) <= new Date()) {
            // We'll keep it in the active bookings list but will display differently in the UI
              ...
              <table className="min-w-full">
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {activeBookings.map((booking) => {
                    ...
                    return (
                      <tr key={booking.id} className={hasEnded ? "bg-gray-50" : ""}>
